...
@torch.no_grad()
def translate_batch_beam(encoder, decoder, sentences, src_vocab, tgt_vocab,
    ...
    B = src.size(0)
    hidden = (h.squeeze(0), c.squeeze(0))  # [B, H]

    results = []
    for i in range(B):
        enc_out_i = enc_out[i:i+1]
        src_mask_i = src_mask[i:i+1]
        h_i = (hidden[0][i:i+1], hidden[1][i:i+1])

        beams = [([SOS_index], h_i, 0.0, False)]
        completed = []

        for _ in range(max_length):
            new_beams = []
            for seq, h_state, score, done in beams:
                if done:
                    new_beams.append((seq, h_state, score, done))
                    continue

                y_t = torch.tensor([seq[-1]], dtype=torch.long, device=device)
                logp, new_hidden, _ = decoder.forward_step(y_t, h_state, enc_out_i, src_mask_i)
                topv, topi = logp.topk(beam_size)
                for k in range(beam_size):
                    next_token = topi[0, k].item()
                    next_score = score + topv[0, k].item()
                    next_seq = seq + [next_token]
                    done_flag = (next_token == EOS_index)
                    new_beams.append((next_seq, new_hidden, next_score, done_flag))

            beams = sorted(new_beams, key=lambda x: x[2], reverse=True)[:beam_size]
            completed.extend([b for b in beams if b[3]])
            if all(b[3] for b in beams):
                break

        if completed:
            best_seq = max(completed, key=lambda x: x[2])[0]
        else:
            best_seq = beams[0][0]

        words = [tgt_vocab.index2word.get(tok, '<unk>') 
                 for tok in best_seq[1:] if tok != EOS_index]
...
def translate_batch_beam(encoder, decoder, sentences, src_vocab, tgt_vocab,
    ...
    for s in sentences:
        example = Example(tensorize_sentence(src_vocab, s), [EOS_index])
        src, src_lens, src_mask, _, _, _ = collate_examples([example])
        enc_out, (h, c) = encoder(src, src_lens)
        hidden = (h.squeeze(0), c.squeeze(0))  # [1, H]

        beams = [([SOS_index], (hidden[0].clone(), hidden[1].clone()), 0.0, False)]
        completed = []
        ...
        for _ in range(max_length):
            ...
            for seq, h_state, score, done in beams:
                ...
                y_t = torch.tensor([seq[-1]], dtype=torch.long, device=device)
                logp, new_hidden, _ = decoder.forward_step(
                    y_t, h_state, enc_out, src_mask
                )
                topv, topi = logp.topk(beam_size)
                for k in range(beam_size):
                    next_token = topi[0, k].item()
                    ...
                    next_seq = seq + [next_token]
                    done_flag = (next_token == EOS_index)
                    new_beams.append((next_seq, (new_hidden[0].clone(), new_hidden[1].clone()), next_score, done_flag))
        ...
        if completed:
            ...
            best_seq = beams[0][0]
        words = [
            tgt_vocab.index2word.get(tok, '<unk>')
            for tok in best_seq[1:] if tok != EOS_index
        ]
        all_results.append(' '.join(words))

    return all_results

@torch.no_grad()
def bleu_on_pairs(encoder, decoder, pairs, src_vocab, tgt_vocab, max_eval=200, beam_size=5):
    encoder.eval()
    decoder.eval()
    ...
    for s, t in sample:
        out = translate_batch_beam(encoder, decoder, [s], src_vocab, tgt_vocab, beam_size=beam_size)
        cands.append(clean(out[0]))
        refs.append([clean(t).split()])

    cands_clean = [c.split() for c in cands]
...
def main():
    ...
    for s, _ in split_lines(args.test_file):
        ...
        if len(batch) == args.batch_size:
            outs = translate_batch_beam(encoder, decoder, batch, src_vocab, tgt_vocab, beam_size=5)
